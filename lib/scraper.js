                ...
                return ph.createPage().then((page) => {
                    ...
                    return page.open(url).then((status) => {
                        ...
                        $(".comment-thread-renderer").each(() => {
                            const root = $(this).children().first();
                            const children = [];
                            $(".comment-replies-renderer .comment-replies-renderer-pages .comment-renderer", this).each((i) => {
                                const author = $(".comment-renderer-header", this).children().first();
                                const child = {
                                ...
                                const receiver = $(".comment-renderer-text-content", this).find("a").text();
                                if (receiver !== "") {
                                    child.receiver = receiver;
                                }
                                children.push(child);
                            });
                            const author = $(".comment-renderer-header", root).children().first();
                            const comment = {
